var App = function() {
    ...
    var dashboard = function() {
        ...
        if ($('#site_statistics').size() != 0) {
            ...
            $.plot('#count-user', count_user_data, {
                series: {
                    pie: {
                        show: true,
                        innerRadius: 0.27,
            ...
            $.plot('#count-group', count_group_data, {
                series: {
                    pie: {
                        show: true,
                        innerRadius: 0.27,
        ...
        init: function(options) {
            ...
            if (config.bootstrapSwitch) {
                ...
                $('.switch').on('switchChange.bootstrapSwitch',function (e,state) {
                    var _this = $(this);
                    _this.bootstrapSwitch('state', !state, true);
                    layer.confirm('确定切换状态吗？', {
                        btn: ['确定','取消'] //按钮
                    }, function(index){
                        layer.close(index);
                        var form = $("#active-"+_this.data('model')+"-form-"+_this.data('id'));
                        var is_active = 0;
                        if(state==true){
                            is_active = 1;
                        }
            ...
            if (config.select2) {
                $(".select2").each(function (index,item) {
                    var selectoObj = $(this).select2({
                        width: '100%'
                    });
                    var vals = $(this).data('val');
                    if(vals){
                        selectoObj.val(vals.toString().split(',')).trigger('change');
                    }
                })
